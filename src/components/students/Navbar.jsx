...
const Navbar = () => {
  ...
  const { isEducator } = useContext(AppContext); // ✅ Access context separately
  ...
  return (
    ...
    lg:px-36 border-b border-gray-500 py-4 ${isCourseListPage ? 'bg-white' : 'bg-cyan-100/70'}`}>
      ...
      <div className="hidden md:flex items-center gap-5 text-gray-500">
        ...
        {
          user ? <UserButton /> :
            <button onClick={() => openSignIn()} className='bg-blue-500 text-white px-4 py-2 rounded-full hover:bg-blue-600'>Create Account</button>
        }
      </div>
